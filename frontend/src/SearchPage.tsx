...
import { api } from './api'
import type { Book, CrawlStatus, SyosetuResult } from './types'

// 输入形如作品/章节链接或纯 N コード时直接爬取，否则按关键词搜索
...
export default function SearchPage() {
  ...
  const [results, setResults] = createSignal<SyosetuResult[] | null>(null)
  ...
  const startFetch = async (url: string) => {
    setError('')
    try {
      const book = await api.syosetuFetch(url)
  ...
  const submit = async () => {
    ...
    setResults(null)
    try {
      const r = await api.syosetuSearch(q)
  ...
  const timer = setInterval(async () => {
    ...
    const updated = await Promise.all(pending.map(async (j) => {
      try {
        return { book: j.book, status: await api.syosetuStatus(j.book.id) }
      } catch {
        return j
      }
    }))
  ...
  return (
    <div class="max-w-[860px]">
      ...
      <p class="text-[13px] text-muted mb-4">
        数据来源：syosetu.com（成为小说家吧）。输入关键词搜索，或直接粘贴作品链接
        / N コード爬取。为规避风控，抓取为串行限速，长书耗时较长。
      </p>
                ...
                <div class="flex items-center justify-between gap-3">
                  ...
                  <div class="shrink-0 flex gap-2">
                    <Show when={j.status?.running}>
                      <button class="text-[13px]" onClick={() => api.syosetuStop(j.book.id)}>停止</button>
                    </Show>
